...
function worldsPlugin(): Plugin {
  ...
  const TEMPORAL_PROJECT_VERSION = 2
  ...
  function sanitizePlacementProject(input: unknown) {
    ...
    if ((record.version !== PROJECT_VERSION && record.version !== TEMPORAL_PROJECT_VERSION) || !Array.isArray(record.instances)) return undefined
    ...
    const normalizedShadowCatcherColor = typeof shadowCatcherColor === 'string' && /^#[0-9a-f]{6}$/i.test(shadowCatcherColor)
      ...
      : undefined
    const temporalScene = (() => {
      if (!record.temporalScene || typeof record.temporalScene !== 'object') return undefined
      const temporal = record.temporalScene as Record<string, unknown>
      const bounds = temporal.bounds as Record<string, unknown> | undefined
      const camera = temporal.camera as Record<string, unknown> | undefined
      if (
        temporal.version !== 1 ||
        !bounds || typeof bounds.width !== 'number' || typeof bounds.depth !== 'number' || typeof bounds.groundY !== 'number' ||
        !camera || !isVec3(camera.position) || !isVec3(camera.target) || typeof camera.fov !== 'number' ||
        !Array.isArray(temporal.nodes) || !Array.isArray(temporal.relations) || !Array.isArray(temporal.anchors)
      ) return undefined
      const validKinds = new Set(['road', 'building', 'tree', 'hedge', 'utility-box', 'moped', 'person', 'signal', 'prop'])
      const validLayers = new Set(['foreground', 'midground', 'background'])
      const isUnitNumber = (value: unknown): value is number => typeof value === 'number' && Number.isFinite(value) && value >= 0 && value <= 1
      const isNormalizedBox = (value: unknown): value is [number, number, number, number] => (
        Array.isArray(value) && value.length === 4 && value.every(isUnitNumber)
      )
      const readImageAnchor = (value: unknown) => {
        if (!value || typeof value !== 'object') return undefined
        const anchor = value as Record<string, unknown>
        const point = anchor.point
        if (!Array.isArray(point) || point.length !== 2 || !point.every(isUnitNumber)) return undefined
        if (!isUnitNumber(anchor.confidence)) return undefined
        return {
          point: point as [number, number],
          ...(isUnitNumber(anchor.heightFactor) ? { heightFactor: anchor.heightFactor } : {}),
          confidence: anchor.confidence,
          ...(typeof anchor.source === 'string' ? { source: anchor.source } : {}),
        }
      }
      const nodes = temporal.nodes.flatMap((candidate) => {
        if (!candidate || typeof candidate !== 'object') return []
        const node = candidate as Record<string, unknown>
        if (
          typeof node.id !== 'string' || !validKinds.has(String(node.kind)) || !validLayers.has(String(node.layer)) ||
          !isVec3(node.position) || !Array.isArray(node.footprint) || node.footprint.length !== 2 ||
          !node.footprint.every((part) => typeof part === 'number' && Number.isFinite(part)) ||
          typeof node.height !== 'number' || typeof node.importance !== 'number' || typeof node.role !== 'string'
        ) return []
        const imageAnchor = readImageAnchor(node.imageAnchor)
        return [{
          id: node.id, kind: node.kind, role: node.role, layer: node.layer, position: node.position,
          footprint: node.footprint, height: node.height, importance: node.importance,
          ...(isVec3(node.rotation) ? { rotation: node.rotation } : {}),
          ...(isVec3(node.scale) ? { scale: node.scale } : {}),
          ...(typeof node.color === 'string' && /^#[0-9a-f]{6}$/i.test(node.color) ? { color: node.color.toLowerCase() } : {}),
          ...(isNormalizedBox(node.imageBox) ? { imageBox: node.imageBox } : {}),
          ...(imageAnchor ? { imageAnchor } : {}),
          ...([0, 1, 2, 3].includes(node.visualPriority as number) ? { visualPriority: node.visualPriority } : {}),
          ...(isUnitNumber(node.confidence) ? { confidence: node.confidence } : {}),
        }]
      })
      const nodeIds = new Set(nodes.map((node) => node.id))
      if (!nodes.length || nodeIds.size !== nodes.length) return undefined
      const relations = temporal.relations.flatMap((candidate) => {
        if (!candidate || typeof candidate !== 'object') return []
        const relation = candidate as Record<string, unknown>
        if (
          !['occludes', 'supports', 'clustered-with', 'behind', 'frames'].includes(String(relation.type)) ||
          typeof relation.from !== 'string' || typeof relation.to !== 'string' ||
          !nodeIds.has(relation.from) || !nodeIds.has(relation.to) ||
          typeof relation.strength !== 'number' || relation.strength < 0 || relation.strength > 1
        ) return []
        return [{ type: relation.type, from: relation.from, to: relation.to, strength: relation.strength }]
      })
      const anchors = temporal.anchors.flatMap((candidate) => {
        if (!candidate || typeof candidate !== 'object') return []
        const anchor = candidate as Record<string, unknown>
        if (typeof anchor.year !== 'number' || !Number.isFinite(anchor.year) || !anchor.patches || typeof anchor.patches !== 'object') return []
        const patches = Object.entries(anchor.patches as Record<string, unknown>).flatMap(([id, patch]) => {
          if (!nodeIds.has(id) || !patch || typeof patch !== 'object') return []
          const value = patch as Record<string, unknown>
          if (!isVec3(value.position) && !isVec3(value.rotation) && !isVec3(value.scale) && typeof value.opacity !== 'number') return []
          return [[id, {
            ...(isVec3(value.position) ? { position: value.position } : {}),
            ...(isVec3(value.rotation) ? { rotation: value.rotation } : {}),
            ...(isVec3(value.scale) ? { scale: value.scale } : {}),
            ...(typeof value.opacity === 'number' ? { opacity: Math.min(Math.max(value.opacity, 0), 1) } : {}),
          }]]
        })
        return [{ year: anchor.year, patches: Object.fromEntries(patches) }]
      }).sort((a, b) => a.year - b.year)
      const evaluation = temporal.evaluation as Record<string, unknown> | undefined
      if (!evaluation || evaluation.target !== 'overall-recognizability' || typeof evaluation.fixedView !== 'boolean' || !Array.isArray(evaluation.checks) || !evaluation.checks.every((check) => typeof check === 'string')) return undefined
      return {
        version: 1,
        ...(typeof temporal.sourceImage === 'string' ? { sourceImage: temporal.sourceImage } : {}),
        bounds: { width: bounds.width, depth: bounds.depth, groundY: bounds.groundY },
        camera: { position: camera.position, target: camera.target, fov: camera.fov },
        nodes,
        relations,
        anchors,
        evaluation: { target: 'overall-recognizability', fixedView: evaluation.fixedView, checks: evaluation.checks },
      }
    })()

    return {
      version: temporalScene ? TEMPORAL_PROJECT_VERSION : PROJECT_VERSION,
      instances,
      ...(sun ? { sun } : {}),
