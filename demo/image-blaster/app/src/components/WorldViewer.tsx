...
import { TemporalSandbox } from '../modules/temporal/TemporalSandbox'
...
export function WorldViewer({
  ...
  const activePreviewAlt = hoveredObjectAsset
    ...
    : hoveredWorldPreview?.imageUrl
      ...
      : 'Original source'
  const temporalScene = sceneProject?.temporalScene
  const temporalSandboxActive = Boolean(temporalScene)
  return (
    <>
      <Canvas
      ...
      >
        <Suspense fallback={null}>
          ...
          {temporalScene && <TemporalSandbox scene={temporalScene} year={temporalYear} calibration={calibrationMode} imageAspect={calibrationImageAspect} />}
          <Physics key={`${desiredSlug}:${controllerResetToken}`} gravity={[0, -9.81, 0]}>
            {!temporalSandboxActive && activeControllerMode === 'fly' ? (
              <FlyController ref={charRef as React.RefObject<FlyControllerHandle>} preserveCameraOnMount={editing} />
            ) : !temporalSandboxActive ? (
              <CharacterController ref={charRef as React.RefObject<CharacterControllerHandle>} />
            ) : null}
            {!temporalSandboxActive && showScene && colliderUrl && (
            ...
            {!temporalSandboxActive && showObjects && !editing && (
              <Suspense fallback={null}>
                <ObjectGrid
          ...
          {!temporalSandboxActive && splatUrl && (
          ...
          {!temporalSandboxActive && !panoUrl && <DefaultEnvironment intensity={activeEnvironmentIntensity} />}
