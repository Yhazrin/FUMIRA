...
import { TemporalTimeline } from './modules/temporal/TemporalTimeline'
import { CalibrationOverlay } from './modules/calibration/CalibrationOverlay'
...
function LoadedApp({
  ...
  const [calibrationAspect, setCalibrationAspect] = useState(4 / 3)
  ...
  const activeTemporalScene = sceneProjectActive ? sceneProject?.temporalScene : undefined
  ...
  useEffect(() => {
    const firstYear = activeTemporalScene?.anchors[0]?.year
    if (firstYear !== undefined) setTemporalYear(firstYear)
  }, [activeTemporalScene, entry.slug])
  ...
  return (
    <div className="relative w-screen h-screen bg-black overflow-hidden select-none [&_*]:focus:outline-none [&_*]:focus-visible:outline-none [&_*]:focus:ring-0 [&_*]:focus-visible:ring-0">
      ...
      <WorldViewer
        ...
        calibrationMode={calibrationMode && Boolean(activeTemporalScene)}
        calibrationImageAspect={calibrationAspect}
      />
      {!editing && uiVisible && emptyWorld && !activeTemporalScene && (
      ...
      {uiVisible && activeTemporalScene && calibrationMode && (
        <CalibrationOverlay
          scene={activeTemporalScene}
          year={temporalYear}
          sourceImageUrl={entry.sourceImageUrl}
          imageAspect={calibrationAspect}
          onImageAspect={setCalibrationAspect}
        />
      )}
      {uiVisible && activeTemporalScene && (
        <TemporalTimeline
          scene={activeTemporalScene}
          year={temporalYear}
          onYearChange={setTemporalYear}
          calibration={calibrationMode}
          onCalibrationToggle={() => setCalibrationMode((mode) => !mode)}
        />
      )}
      {!editing && uiVisible && (
        <>
